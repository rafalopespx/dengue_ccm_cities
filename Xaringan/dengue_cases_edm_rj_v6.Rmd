...
knitr::include_graphics('img_v5/patch_final_surrogates.png')
```
...
We test four different S-Map models, every one of each follows the same procedure, put the drivers, finds a minimum for the $\theta$ and find the forces of interactions:

 - Models until 52 weeks of lags
    * (Model I) S-Map for 
    $\langle Cases(t), Precip_{max}(t-49), Precip_{min}(t-25), Precip_{mean}(t-49)\rangle$
    * (Model II) S-Map for 
    $\langle Cases(t), Precip_{max}(t-49), Precip_{min}(t-25), Precip_{mean}(t-49), Temp_{min}(t)\rangle$

- Models until 17 weeks (First 0 of ACF of Cases time series) of lags
    * (Model III) S-Map for 
    $\langle Cases(t), Precip_{max}(t-2), Precip_{min}(t-7))\rangle$
    * (Model IV) S-Map for 
    $\langle Cases(t), Precip_{max}(t-2), Precip_{min}(t-7), Temp_{min}(t)\rangle$
...
```{r theta-modelI, echo=FALSE, message=FALSE, warning=FALSE, out.width=750, fig.align='center'}
knitr::include_graphics('img_v6/yearly_shuffle_tp_52_theta_min_mae_rmse.png')
```
...
With the value of theta for the minimum on MAE, we can ow estimate the forces of interaction, $C_i(t)$ for each drivers ( $Precip_{min}$, $Precip_{max}$, $Precip_{mean}$ ) on the the effect:

$$
\begin{eqnarray}
Cases(t) &=& C_0(t) + \frac{\partial Precip_{min}(t - 25)}{\partial Cases(t)}Precip_{min}(t - 25) \\
&+& \frac{\partial Precip_{max}(t - 49)}{\partial Cases(t)}Precip_{max}(t - 49) \\
&+& \frac{\partial Precip_{mean}(t - 49 )}{\partial Cases(t)}Precip_{mean}(t - 49)
\end{eqnarray}
$$

---

```{r forces-modelI, echo=FALSE, message=FALSE, warning=FALSE, out.width=750, fig.align='center'}
knitr::include_graphics('img_v6/patchwork_coef_precip_tp_52.png')
```
...
```{r theta-modelII, echo=FALSE, message=FALSE, warning=FALSE, out.width=750, fig.align='center'}
knitr::include_graphics('img_v6/yearly_shuffle_tp_52_add_temp_min_theta_min_mae_rmse.png')
```
...
With the value of theta for the minimum on MAE, we can ow estimate the forces of interaction, $C_i(t)$ for each drivers ( $Precip_{min}$, $Precip_{max}$, $Precip_{mean}$, $Temp_{min}$ ) on the the effect:
...
&+& \frac{\partial Precip_{max}(t - 49)}{\partial Cases(t)}Precip_{max}(t - 49) \\
&+& \frac{\partial Precip_{mean}(t - 49 )}{\partial Cases(t)}Precip_{mean}(t - 49) \\ 
&+& \frac{\partial Temp_{min}(t)}{\partial Cases(t)}Temp_{min}(t)
\end{eqnarray}
$$

---

```{r forces-modelII, echo=FALSE, message=FALSE, warning=FALSE, out.width=750, fig.align='center'}
knitr::include_graphics('img_v6/patchwork_coef_precip_tp_52_add_tmin.png')
```
...
knitr::include_graphics('img_v6/yearly_shuffle_tp_17_theta_min_mae_rmse.png')
```
...
$$
\begin{eqnarray}
Cases(t) &=& C_0(t) + \frac{\partial Precip_{max}(t - 2)}{\partial Cases(t)}Precip_{max}(t - 2) \\
&+& \frac{\partial Precip_{min}(t - 7)}{\partial Cases(t)}Precip_{min}(t - 7)
\end{eqnarray}
$$

---

```{r forces-modelIII, echo=FALSE, message=FALSE, warning=FALSE, out.width=750, fig.align='center'}
knitr::include_graphics('img_v6/patchwork_coef_precip_tp_17.png')
```
...
```{r theta-modelIV, echo=FALSE, message=FALSE, warning=FALSE, out.width=750, fig.align='center'}
knitr::include_graphics('img_v6/yearly_shuffle_tp_17_add_temp_min_theta_min_mae_rmse.png')
```
...
With the value of theta for the minimum on MAE, we can ow estimate the forces of interaction, $C_i(t)$ for each drivers ( $Precip_{min}$, $Precip_{max}$, $Temp_{min}$ ) on the the effect:

$$
\begin{eqnarray}
Cases(t) &=& C_0(t) + \frac{\partial Precip_{min}(t - 2)}{\partial Cases(t)}Precip_{min}(t - 2) \\
&+& \frac{\partial Precip_{max}(t - 7)}{\partial Cases(t)}T_{mean}(t - 7) \\
&+& \frac{\partial Temp_{min}(t)}{\partial Cases(t)}Temp_{min}(t) \\
\end{eqnarray}
$$
...
```{r forces-modelIV, echo=FALSE, message=FALSE, warning=FALSE, out.width=750, fig.align='center'}
knitr::include_graphics('img_v6/patchwork_coef_precip_tp_17_add_tmin.png')
```
 ...
 * __Minimum Temperature__, __Mean Temperature__ and __Minimum Preciptation__ are climate drivers of Cases at Rio de Janeiro cit, but it seasonality and not by its variability, so they are a conditions that is matched by season on year. When in the range that favourable to the mosquito cycle, Temperatures shorts the Extrinsic Incubation period, this is a contribution always positive and above suitable temperature for transmission.
