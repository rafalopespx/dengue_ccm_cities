...
drivers_coef_opt_mae<-vroom(paste0("Outputs/Tables/rj/yearly_shuffle_drivers_tp_", 
                                   tp, 
...
## Coefficients
coef_tmin<-drivers_coef_opt_mae |> 
...
coef_tmin_colors<-drivers_coef_opt_mae %>% 
  coef_plot(xvar = temp_min52, yvar = `∂temp_min52/∂cases`, colors = month(date - 2))+
  labs(x = "Minimum Temperature [ºC], lagged 2 weeks", 
       title = "Effects of Minimum Temperature on Cases")+
